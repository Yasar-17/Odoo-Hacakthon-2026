...
const config: Record<string, { bg: string; text: string; dot: string; label: string }> = {
  ...
  ACTIVE:    { bg: "bg-secondary-container", text: "text-on-secondary-container", dot: "bg-primary", label: "Active" },
  ...
  ON_LEAVE:  { bg: "bg-surface-container-high", text: "text-on-surface-variant", dot: "bg-outline", label: "On Leave" },
};

export default function StatusBadge({ status }: { status: Status }) {
  const s = config[status] ?? { bg: "bg-surface-container", text: "text-secondary", dot: "bg-outline", label: status };
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-label-md ${s.bg} ${s.text}`}>
